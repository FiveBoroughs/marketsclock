...
function Clock(props) {
    ...
    const TimeFitText = useFitText();
    const SessionFitText = useFitText();
    const NextSessionFitText = useFitText();
    ...
    return (
        <ClockContainer>
            ...
            <Title ref={TitleFitText.ref} style={{ fontSize: TitleFitText.fontSize }}>
                {props.market.Title}
            </Title>
            {
                props.isLoading ? (
                    <Loader>Loading</Loader>
                        ...
                        <div style={{ height: '75%', width: '100%' }}>
                            <TimeText ref={TimeFitText.ref} style={{ fontSize: TimeFitText.fontSize }}
                                sessionColor={currentSession && currentSession.Style}>
                                {localedTime && localedTime.format('LTS')}
                            </TimeText>
                            <Session ref={SessionFitText.ref} style={{ fontSize: SessionFitText.fontSize }}
                                sessionColor={currentSession && currentSession.Style}>
                                {currentSession && currentSession.Title}
                            </Session>
                            <NextSession ref={NextSessionFitText.ref} style={{ fontSize: NextSessionFitText.fontSize }}>
